...
# Cobalamin Stacked Barplots - Frag ---------------------------------------
...
# Note: Does not account for cobamers below LOD/LOQ
# Create a summary table with mean values by sample per molC
frag_cobal_data_sum_nmol_molC <-
  frag_cobal_quant_data %>% 
...
# Create a summary table with mean values by sample per molC
frag_cobal_data_sum_attomoles_cell <-
  frag_cobal_quant_data %>% 
  group_by(Replicate.Name, B12.Treatment) %>% 
  summarise(total_attomoles_cell = sum(mean_attomoles_cell)) %>%
  group_by(B12.Treatment) %>%
  summarise(mean_total_attomoles_cell = mean(total_attomoles_cell))
  

# Bring in LOD/LOQ Info ---------------------------------------------------
...
# Plot for stacked bar with zeptomoles B12/cell in Frag 
...
# Cobalamin Stacked Bar Plots - Pibo --------------------------------------
...
# Loop to notify if below LOD
for (i in 1:nrow(pibo_cobal_quant_data_LODQ)){
  if (pibo_cobal_quant_data_LODQ$Below_LOD[i] == TRUE ){
    pibo_cobal_quant_data_LODQ$mean_nmol_molC[i] <- 0
    pibo_cobal_quant_data_LODQ$mean_molecules.per.cell <- 0
...
# Stacked barplot for pibo
pibo_CobalStackedbar <- ggplot() +
  geom_bar(
    data = pibo_plot_data,
